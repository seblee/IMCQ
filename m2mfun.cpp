    ...
    ui(new Ui::M2MFun)
{
    ...
    connect(client, &QMqttClient::messageReceived,      this, &M2MFun::getMessage);
...
void M2MFun::on_pushButtonSend_clicked()
{
    if (client->state() == QMqttClient::Connected)
    {
        if (ui->textEditSend->toPlainText().isEmpty())
        {
            QToolTip::showText(QCursor::pos(),
                               tr("Message to be send cannot be empty"),
                               ui->pushButtonSend);
            return;
        }
        QJsonObject json;
        ...
        json.insert(tr("Message"),
                    QJsonValue(ui->textEditSend->toPlainText()));
        ...
        else
        {
            ...
            content.append(tr("%1(%2)\r")
                           .arg(deviceName)
                           .arg(QDateTime::currentDateTime().toUTC().toString(Qt::ISODate))
                           );


            content.append(ui->textEditSend->toPlainText() + tr("\r\r"));


            QColor clrR(5, 150, 0);
            stringToHtmlFilter(content);
            stringToHtml(content, clrR);
            ui->textBrowserReceive->moveCursor(QTextCursor::End);
            ui->textBrowserReceive->insertHtml(content);
            ui->textBrowserReceive->moveCursor(QTextCursor::End);
    ...
    {
        // ui->statusbar->showMessage(tr("Client is not connected"));
    }
}
...
void M2MFun::getMessage(const QByteArray& message, const QMqttTopicName& topic)
{
    QJsonParseError jsonError;
    ...
    ui->textEditLog->append(QString::fromUtf8(message));
    ...
    if (!doucment.isNull() && (jsonError.error == QJsonParseError::NoError))
    {
        ...
        {
            QString Receiver = object.value("Receiver").toVariant().toString();
            qDebug() << tr("Receiver:") << Receiver;

            if (QString::compare(Receiver, deviceName, Qt::CaseSensitive) != 0)
            {
                qDebug() << tr("Wrong Receiver");
                return;
            }
        }
        else
        {
            qDebug() << tr("Data error");
            return;
        }

        if (object.contains("Sender"))
        {
            QString Sender = object.value("Sender").toVariant().toString();
            qDebug() << tr("Sender:") << Sender;
            content.append(tr("%1(%2)\r")
                           .arg(Sender)
                           .arg(QDateTime::currentDateTime().toUTC().toString(Qt::ISODate))
                           );
        ...
        if (object.contains("Message"))
        {
            ...
            qDebug() << tr("Message:") << Message;
            content.append(Message + tr("\r\r"));
        }
        else
        {
            qDebug() << tr("Data error");
...
void M2MFun::stringToHtmlFilter(QString& str)
{
...
void M2MFun::stringToHtml(QString& str, QColor crl)
{
    QByteArray array;

    array.append(static_cast<char>(crl.red()));
    array.append(static_cast<char>(crl.green()));
    array.append(static_cast<char>(crl.blue()));
    QString strC(array.toHex());
    str = QString(tr("<span style=\" color:#%1;\">%2</span>").arg(strC).arg(str));
}
